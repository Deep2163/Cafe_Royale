...
function StaffLogin() {
  ...
  const handleLogin = async (values, resetForm) => {
    ...
    try {
      const res = await axios.post("http://localhost:3001/staff/login", {
        email,
        password,
      });
      ...
      localStorage.setItem("staff", res.data.admin._id);

      enqueueSnackbar("staff Login successfully", { variant: "success" });
      navigate("/staff");
      resetForm();
    } catch (error) {
      console.log(error);
      ...
      <section className="admin_login" id="admin_login_sec">
        <div className="container-fluid position-relative">
          <div className="row form_popup ">
            <div className="col-md-6 col-12 order-md-1 order-2 d-flex flex-column aligin-items-center justify-content-center">
              <div className="admin_login_form text-start ">
                <h2 className="fw-bold text-black  welcome_back fs-1 mb-4 text-center">
                  Welcome Staff
                </h2>
                <h6 className="text-black fs-4 text-center">
